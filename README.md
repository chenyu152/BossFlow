...
- **人在回路** — 打招呼发送、简历覆盖等关键动作必须用户确认，AI 只做建议不做决定
- **可审计** — AI 生成内容标注证据来源（JD / 简历 / 项目代码），不凭空编造
...
| **Pipeline** | 待处理队列、粗筛打分、LLM 精评、状态流转、删除 |
