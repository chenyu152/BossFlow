  ...
  return (
    <section className="account-activity-page h-full overflow-auto p-4 lg:p-6">
      <div className="mx-auto max-w-[1500px] space-y-3">
        ...
        {loginUnavailable ? <div className="account-activity-login-alert account-activity-login-alert--error"><ShieldAlert size={18} /><div><strong>{loginChecking ? '正在检查 BOSS 登录状态' : loginState?.serverInvalidated ? '登录已失效，请重新登录' : '登录已失效/未保存 Cookie'}</strong><span>{loginChecking ? '正在确认指定账号 Profile，页面内容仍可查看。' : loginState?.serverInvalidated ? 'BOSS 服务端已拒绝当前会话，导入新岗位无法获取完整 JD，请重新登录并保存 Cookie。' : '导入新岗位无法获取完整 JD，请先登录并保存 Cookie。'}</span></div><button type="button" onClick={onOpenLoginSettings}>前往系统设置</button></div> : loginState?.status === 'refresh_recommended' ? <div className="account-activity-login-alert account-activity-login-alert--warning"><ShieldAlert size={18} /><div><strong>建议刷新 BOSS 登录</strong><span>当前 Cookie 仍可用，但已达到刷新建议时间。</span></div><button type="button" onClick={onOpenLoginSettings}>前往系统设置</button></div> : null}
